...
def generate_variations(seed: dict, n: int = VARIATIONS_PER_SEED, api_key: str | None = None, backend: str | None = None) -> list[dict]:
...
Return a JSON array of {n} objects with keys: question, ground_truth_sql, notes."""
    ...
    text = chat(MODEL, SYSTEM_PROMPT, prompt, max_tokens=2000, cache_system=True, api_key=api_key, backend=backend)
    if text.startswith("```"):
        text = text.split("```")[1]
...
def build_golden_dataset(api_key: str | None = None, backend: str | None = None) -> None:
    ...
    for i, seed in enumerate(seeds, 1):
        ...
        try:
            variations = generate_variations(seed, api_key=api_key, backend=backend)
